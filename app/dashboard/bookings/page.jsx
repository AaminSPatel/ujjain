...
import { Calendar, Search, Filter, MoreHorizontal, Eye, CheckCircle, XCircle, Plus, Edit, Trash2, X, User, Mail, Phone, MapPin, Clock, CreditCard, Info, Grid, List } from "lucide-react"
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredBookings.map((booking) => (
                      ...
                      >
                        <TableCell className="font-medium">{booking._id?.slice(-8)}</TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium">{booking.user?.fullName || "N/A"}</div>
                            <div className="text-sm text-muted-foreground">{booking.user?.email || "N/A"}</div>
                          </div>
                        </TableCell>
                        <TableCell>{getServiceName(booking)}</TableCell>
                        <TableCell className="capitalize">{booking.serviceType || "N/A"}</TableCell>
                        <TableCell>
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <h2 className="font-bold text-lg">{getServiceName(booking)}</h2>
                        </div>
                        <Badge className={statusColors[booking.status] || statusColors.pending}>
                          {booking.status || "pending"}
                        </Badge>
                      ...
                      <div className="space-y-2 mb-4">
                        <div>
                          <p className="text-sm font-medium">{booking.user?.fullName || "N/A"}</p>
                          <p className="text-sm text-muted-foreground">{booking.user?.email || "N/A"}</p>
                        </div>
                        
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">Type</span>
                          <span className="text-sm font-medium capitalize">{booking.serviceType || "N/A"}</span>
                        </div>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-sm font-medium">₹{booking.payment?.amount || 0}</span>
                        </div>
                      </div>
                      ...
                      <Card>
                        ...
                        <CardContent className="grid gap-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium">{selectedBooking.user?.fullName || "N/A"}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-muted-foreground flex items-center gap-1">
                              <Mail className="h-4 w-4" /> Email
                            </span>
                            <span className="font-medium">{selectedBooking.user?.email || "N/A"}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-muted-foreground flex items-center gap-1">
                              <Phone className="h-4 w-4" /> Phone
                            </span>
                            <span className="font-medium">{selectedBooking.user?.mobile || "N/A"}</span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="grid gap-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium capitalize">{selectedBooking.serviceType || "N/A"}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-muted-foreground">Service</span>
                            <span className="font-medium">{getServiceName(selectedBooking)}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-muted-foreground">Duration</span>
                            <span className="font-medium">
                              {formatDate(selectedBooking.startDate)} 
                              {selectedBooking.endDate && ` to ${formatDate(selectedBooking.endDate)}`}
                            </span>
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="grid gap-2">
                          <div className="flex items-start justify-between">
                            ...
                            <span className="font-medium text-right">{selectedBooking.pickupLocation?.address || "N/A"}</span>
                          </div>
                          {selectedBooking.dropoffLocation && (
                            <div className="flex items-start justify-between">
                              <span className="text-muted-foreground">Dropoff</span>
                              <span className="font-medium text-right">{selectedBooking.dropoffLocation.address}</span>
                            </div>
                          )}
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="grid gap-2">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="font-medium">₹{selectedBooking.payment?.amount || 0}</span>
                          </div>
                          <div className="flex items-center justify-between">
                            <span className="text-muted-foreground">Method</span>
                            <span className="font-medium capitalize">
                              {getPaymentMethodText(selectedBooking.payment?.method) || "N/A"}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                          ...
                          {selectedBooking.payment?.transactionId && (
                            <div className="flex items-center justify-between">
                              ...
                              <span className="font-medium text-xs">{selectedBooking.payment.transactionId}</span>
                            </div>
                          )}
                        </CardContent>
                      </Card>
